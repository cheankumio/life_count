...
public class signUpUser extends AppCompatActivity {
    ...
    public void setElemtent(){
        ...
        sign_btn = (Button)findViewById(R.id.button);
    }

    @Override
    protected void onDestroy() {

        //Log.d("MYLOG","Destroy result_code: "+result_code);
        super.onDestroy();
    }

    //讀取使用者資訊，並存入SharePreferences
    public void savedata(View view){
        if(muserName.getText().length()>0 && mwishAge.getText().length()>0) {
            // 啟動Realm 資料庫
            ...
            result_code = 66;
            Log.d("MYLOG","result_code: "+result_code);
            this.setResult(RESULT_OK);
            finish();
        }else{
            Toast.makeText(this, "請確認資料皆已填入", Toast.LENGTH_SHORT).show();
